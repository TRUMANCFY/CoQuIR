"""
Add Instructor model, referring to https://huggingface.co/hkunlp/instructor-large
"""
...
class InstructorTransformer(Transformer):
    def __init__(
        ...
        if "bigtr" in model_name_or_path and os.path.isdir(model_name_or_path):
            ...
            config = AutoConfig.from_pretrained(
                model_name_or_path, **model_args, cache_dir=None
            )
...
class InstructorRetriever:
    def __init__(self, model_name):
        self.model = Instructor(model_name)

    def encode_queries(self, sentences, batch_size: int, **kwargs) -> np.ndarray:
        input_texts = []
        
        # hardcode for InstructIR
        if '[SEP]' in sentences[0]:
            for sentence in sentences:
                assert '[SEP]' in sentence
                query_chunks = sentence.split('[SEP]')
                assert len(query_chunks) == 2
                input_texts.append([component.strip() for component in query_chunks])
        # hardcode for PIR
        else:
            for sentence in sentences:
                ...
                if len(sentence_splits) == 2:
                    input_texts.append(sentence_splits)
                elif len(sentence_splits) == 1:
                    # AmbigQA
                    if "instruction" in kwargs and kwargs["instruction"]:
                        _prompt = kwargs["instruction"]
                    else:                            
                        _prompt = ""
                    print("prompt: ", _prompt)
                    input_texts.append([_prompt, sentence_splits[0]])
                else:
                    input_texts.append([sentence_splits[0], ' '.join(sentence_splits[1:])])
    ...
    def encode_corpus(self, corpus, batch_size: int, **kwargs) -> np.ndarray:
        input_texts = []
        for doc in corpus:
            input_texts.append(["Represent the code for retrieval: ", f"{doc.get('title', '')} {doc['text']}"])
